...
// 从文件路径读取图像进行滑块匹配
func SlideMatchWithPath(targetImagePath, backgroundImagePath string) (*SlideBBox, error) {
...
// 从文件路径读取图像进行简单滑块匹配
func SimpleSlideMatchWithPath(targetImagePath, backgroundImagePath string) (*SlideBBox, error) {
	targetData, err := os.ReadFile(targetImagePath)
	if err != nil {
		return nil, fmt.Errorf("读取目标图像文件失败: %v", err)
	}
